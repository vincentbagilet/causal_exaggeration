...
$$Turnout_{i} = \pi_0 + \pi_1 Rain_{i} + \gamma w_{i} + \epsilon_{i}$$

Where $\pi_0$ is a constant, $Rain$ is either a continuous variable (amount of rain in location $i$ on the day of the election) or a dummy variable (whether it rained or not) and $\epsilon \sim \mathcal{N}(0, \sigma_{\epsilon}^{2})$ noise. Let's refer to $\pi_1$ as "IV strength".
...
generate_data_IV <- function(N,
  ...
  data <- tibble(id = 1:N) %>%
    mutate(
      ...
      epsilon = rnorm(nrow(.), 0, sigma_epsilon),
      turnout = pi_0 + pi_1*rain + gamma*w + epsilon,
...
I also check the standard deviation and means of the variables:

```{r check_sd_mean_IV, code_folding='Show the code used to generate the table'}
...
We want to compare $\dfrac{\mathbb{E}[\hat{\beta}_{IV}]}{\beta_0}$ and $\dfrac{\mathbb{E}[ \hat{\beta}_{IV}|\text{signif}]}{\beta_0}$. The first term represents the bias and the second term represents the exaggeration ratio.

```{r summarise_IV}
summarise_simulations_IV <- function(data) {
  data %>%
    mutate(significant = (p_value <= 0.05)) %>% 
    group_by(delta, pi_1, model, param_rain) %>%
    summarise(
...
summary_sim_IV <- summarise_simulations_IV(sim_IV)
...
summary_sim_IV %>% 
  ...
  ggplot(aes(x = abs(pi_1), y = bias_signif, color = model, linetype = model)) + 
...
summary_sim_IV %>% 
  filter(model %in% c("OLS", "IV")) %>%
  filter(pi_1 >= 0.2) %>% 
  ggplot(aes(x = pi_1, y = bias_signif, color = model)) + 
  geom_line() +
  labs(
    x = "IV strength", 
    y = expression(paste("Average  ", frac("Estimate", "True Effect"))),
...
ggsave("main_graph_IV_paper.pdf", path = "images", width = 8, height = 4.5)
```
...
sim_IV %>% 
  filter(model == "IV") %>% 
  mutate(significant = (p_value <= 0.05)) %>% 
  ggplot(aes(x = pi_1, y = fstat, color = significant)) +
