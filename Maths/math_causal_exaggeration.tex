...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
		...
		And finally, 
			...
			This variance is close to the one obtained in the case of the naive regression with $\eta_{1}^{2}\sigma_{z}^{2}$ instead of $\sigma_{x}^{2}$ in the denominator. Note that, the following holds:
				~
				\begin{align*}
					\sigma_x^2 & =  \text{Var}(x_{i}) = \text{Var}(\eta_0 + \eta_{1} z_i + \gamma w_i + \epsilon_i) \\
						& = \eta_{1}^{2} \text{Var}(z_i) + \gamma^{2} \text{Var}(w_i) + \text{Var}(\epsilon_{i}) + 2\eta_{1} \gamma \underbrace{\text{cov}(z_i, w_i)}_{0} + 2 \gamma \underbrace{\text{cov}(\epsilon_i, w_i)}_{0} + 2 \eta_{1} \underbrace{\text{cov}(\epsilon_i, z_i)}_{0}\\
						& = \eta_{1}^{2} \sigma_{z}^{2} + \gamma^{2} \sigma_{w}^{2} + \sigma_{\epsilon}^{2} \\
						& < \eta_{1}^{2} \sigma_{z}^{2} 
				\end{align*}
