...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\section{Exaggeration ratio and its properties}
		...
		\subsection{The exaggeration/confounders trade-off}
			...
			The objective of this paper is to compare ``biases'' resulting from exaggeration and confounders. In this document, we thus aim to compare $E_{\textsc{ovb}}$ to $E_{\textsc{iv}}$. We thus define $\varDelta = E_{\textsc{iv}} - E_{\textsc{ovb}}$ and analyse whether and for which parameters values $\varDelta > 0$. In addition, we study how $\varDelta$ varies with parameter values. 
		~
		\begin{multline*}
			\varDelta = 
				\dfrac{\sqrt{\sigma_{u}^{2} +  \delta^{2} \sigma_{w}^{2}}}{\beta_{1}\rho_{xz} \sigma_{x}\sqrt{n}}
		...
		First, note that only $E_{\textsc{iv}}$ directly depends on this quantity. Since $E_{\textsc{iv}}$ decreases with $\rho_{xz}$, $\varDelta$ also decreases with $\rho_{xz}$. Now, for fixed and finite values of the other parameters, and since we assumed that $\beta_{1} > 0$,
		~
		\begin{align*}
		...
		Since the other components of $\varDelta$ are finite and do not depend on $\rho_{xz}$, $\lim_{\rho_{xz} \to 0^{+}} \varDelta = + \infty$. Since $\varDelta$ is a continuous function of $\rho_{xz}$ on $\mathbb{R}_{+}^{*}$, for any set of parameter values consistent with our hypotheses, $\exists \rho_{xz} \in (0, 1] \text{ s.t. } E_{\textsc{iv}} > E_{\textsc{ovb}}$. It is also straigthforward to show that since $\eta_{1}\sigma_{z} = \rho_{xz}\sigma_{x}$, for any set of parameter values consistent with our hypotheses, $\exists \eta_{1} \in (0, +\infty) \text{ s.t. } E_{\textsc{iv}} > E_{\textsc{ovb}}$. 
		
		\ul{If the correlation between the instrument and the endogenous variable is limited, the exaggeration bias of the IV will be larger than that of the ``naive'' regression, even in the presence of confounders}.
