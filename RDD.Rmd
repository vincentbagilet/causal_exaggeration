...
The set of parameters may produce test score outside of the range 0-100 in some iterations but that does not affect the analysis. We add the parameter value for delta separately as we will vary the value later and will reuse the table `baseline_param_RDD`.
...
```{r graph_distrib_RDD, fig.asp=0.8, echo=FALSE}
...
graph_distrib_RDD <- sim_rdd %>% 
  ...
  ungroup() %>% 
  ggplot(aes(x = ratio, fill = "All estimates")) +
  ggridges::geom_density_ridges(aes(y = as.factor(bw)), scale = 4)

+ 
  facet_wrap(~ bw, nrow = 9, strip.position = "left", as.table = FALSE) + 
  geom_vline(aes(xintercept = 1)) +
