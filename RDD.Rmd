...
- We assume constant treatment effects.
...
- $U \sim \mathcal{U}(-b_u, b_u)$ the unobserved ability. We assume an uniform distribution for simplicity and because it yields a rather realistic distribution of test scores. Note that a cubic transformation is performed on this uniform distribution, making the effect of ability not uniformly distributed,
...
generate_data_RDD <- function(N, 
  ...
  data <- tibble(id = 1:N) |>  
    mutate(
      u = runif(N, -bound_u, bound_u),
      e_q = rnorm(N, 0, sigma_q),
      qual = alpha_q + delta*u^3 + e_q,
      e_f = rnorm(N, 0, sigma_f),
...
- We assume that the unobserved ability is uniformly distributed between -2.5 and 2.5. We do this tuning by iteration in order to get a desired distribution (with corresponding standard deviation, distribution of test scores, etc). Importantly, such values yield a maximum variation in test scores caused by unobserved ability of $\pm \delta \cdot u^{3} = \pm 30\times (2.5)^3 = \pm 468.75$pts and for each quartile, a variation of
$\pm \delta \cdot u^{3} = \pm 30\times (1.25)^3 \sim \pm 59$pts. The former corresponds to a variation in test scores of more than 2 standard deviations and the later of 0.5 sd.^[Note that unobserved ability explains a large share of the variation in test scores, about 63% <!-- ex_data_RDD |> lm(formula = qual ~ u) |> summary() %>% .$r.squared -->] The case we consider is therefore conservative, with a large OVB. 

The set of parameters may produce test score outside of the range 400–1600 in some iterations but that does not affect the analysis. We add the parameter value for delta separately as we will vary the value later and will reuse the table `baseline_param_RDD`.
...
We want to compare $\mathbb{E}\left[ \left| \frac{\widehat{\beta_{RDD}}}{\beta_0}\right|\right]$ and $\mathbb{E}\left[  \left| \frac{\widehat{\beta_{RDD}}}{\beta_0} \right| | \text{signif} \right]$. The first term represents the bias and the second term represents the exaggeration ratio. This terms depend on the true effect size. To enable comparison across simulations and getting terms independent of effect sizes, we also compute the average of the ratios between the estimate and the true effect, conditional on significance. 
...
summarise_simulations <- function(data) {
  data |>
    ...
    summarise(
      ...
      type_m = mean(ifelse(significant, abs(estimate/true_effect), NA), 
                    na.rm = TRUE),
...
```{r main_graph_RDD, code_folding=TRUE, fig.asp=0.8}
summary_sim_rdd |> 
...
graph_distrib_RDD <- sim_rdd |> 
  ...
  group_by(bw) |> 
  mutate(
    mean_signif_bw = mean(ifelse(p_value < 0.05, abs(ratio), NA), na.rm = TRUE),
  ...
  labs(
    ...
    fill = "",
    title = "Distribution of RDD estimates for 1000 simulated datasets",
...
We chose the values of the variables and parameters to emulate a typical study from this literature. To further check that the results are realistic, we compare the average Signal-to-Noise Ratio (SNR) of our regressions to the range of SNR of an existing study. The closest study to our simulation is [Jacob and Lefgren (2004)](https://direct.mit.edu/rest/article/86/1/226/57489/Remedial-Education-and-Student-Achievement-A). In their main result table, Table 3, the SNRs range from 0.3 to 8.
...
sim_rdd |> 
  group_by(bw) |> 
  summarise(
    # mean_SNR = mean(estimate/se, na.rm = TRUE), 
    median_SNR = median(estimate/se, na.rm = TRUE), 
    .groups = "drop"
  ) |> 
  kable(col.names = c("Bandwidth size", "Median SNR"))
```

The precision of our results is therefore in line with precision that can be found in the literature.
