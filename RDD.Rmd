...
description: "In this document, I run a simulation exercise to illustrate how the use of a Regression Discontinuity Design (RDD) to avoid confounders may lead to a loss in power and inflated effect sizes. To make these simulations realistic, I emulate a typical study estimating the impact of additional lessons on students' grades."
output: 
...
In the present analysis, I build my simulations to replicate a similar type of quasi-experiment. In my fictional example, all students scoring below a cutoff $C$ in a qualification test are required to take additional lessons. I want to estimate the effect of these additional lessons on scores on a final test taken by all students a year later. 

I assume that the final score of student $i$, $Final_i$, is correlated with their qualification score $Qual_i$ and their treatment status $T_i$, *ie* whether student $i$ received additional lessons or not. I further assume that both qualification and final test scores are affected by students' unobserved ability $w_i$ in a non linear way. 
...
<!-- This assumption is not necessary and the results hold if I consider non-constant treatment effects. I thus may drop this assumption in the future. -->
...
- The unobserved availability affects the qualification and final score in a similar way and therefore with the same intensity. However, since the unobserved ability affects  the final score both directly and through the qualification score, I adjust parameters values such that the ability has actually the same effect on both test scores. As a consequence, $\delta = \gamma \times (1 - \eta)$.

More precisely, I set: 

- $N$ the number of students,
- $w \sim \mathcal{U}(-1, 1)$ the unobserved ability. I assume an uniform distribution for simplicity and because it yields a rather realistic distribution of test scores. Note that a cubic transformation is performed on this uniform distribution, making the effect of ability not uniformly distributed,
...
I set baseline values for the parameters to emulate a somehow realistic observational study in this field. I make the following assumptions:

- **Number of observations, bandwidth and cutoff**: to emulate an existing study, I consider a number of observation consistent with @jacob_remedial_2004. In their design, they have 75000 observations per grade and approximately 5000 individuals in their preferred bandwidth, *ie* 6.7% of the total sample size. I choose a slightly smaller but comparable number of observations, **N = 60000**, so that the number of observations in their bandwidth correspond to 0.1 of my sample. In their setting, the cutoff is close to the center of the distribution (roughly 40% of the students get assigned). I consider that the threshold is located at the center of the distribution (**q_c = 0.5**) and a bandwidth that is symmetric around the cutoff in terms of number of observations. A cut-off in the middle of the distribution enables us to consider very wide bandwidths, up to including the whole sample in the analysis.
- **Grades distribution**: I assume that grades are SAT scores. One can think of the qualifying exam as a state level exam that gives the opportunity to students with lower grades to receive additional lessons. For simplicity, I assume that the **scores have a mean of 1060 and standard error of 200** (based on [statistics provided by the Department of Education](https://nces.ed.gov/programs/digest/d20/tables/dt20_226.40.asp)). I adjust the values of the intercept parameters (`mu_q` and `beta_0`) and their standard error (`sigma_epsilon` and `sigma_u`) to get appropriate distributions. These parameters are the last to be set. The resulting set of parameters may produce some test scores outside of the range 400–1600 but that does not affect the analysis. 
- **Correlation between final and qualifying scores**: most of the correlation between these two variables comes from the (omitted) ability. The "direct" parameter $\eta$ only plays a limited role. It makes sense for this parameter to be in the $[0, 1]$ range, closer to the 0 side, considering that I also model "ability". Modifying its value withing this range does not alter the results.
- **Treatment effect size**: typical treatment effect sizes in economics of education are of the order of magnitude of 0.1 sd. @kraft_interpreting_2020, reviewing 747 RCTs of education intervention finds that the median effect size is 0.1 sd. Interestingly, he finds that "larger studies with broad achievement measures have systematically smaller effect sizes". This might hint to publication bias as he underlines. In studies with more than 2000 individuals, the median effect is 0.03 sd. The same median effect size is observed in preregistered studies. I therefore consider an **effect size of 0.03 sd**, *ie* $\beta_1 = 6 = 200\times 0.03$.
- **Modeling of ability**. Modeling ability is rather challenging. Since it is unobserved, there is no actual way of mimicking an existing study to simulate its impact. In the modeling stage, I considered that ability is uniformly distributed between -1 and 1 and affect tests scores through its cube. As a consequence, $\gamma$ takes rather large value. It roughly corresponds to the difference in tests score between the median student and the "best" performing one caused by their differences in ability. We therefore want to set it such that **$\gamma \simeq 300$**. I fine tune it by hand so that it generates enough OVB.
...
Here is an example of data created with my data generating process:
...
Let's check the standard deviation and means of the generated exam scores to make sure that they correspond to what we expect:
...
Then, I quickly look at the distributions of the different variables to check that they have a shape similar to what we expect.
...
I then look at relation between the qualifying score and the unobserved ability. This enables us to understand how where the bias comes form:
...
Note that to run power calculations, we need to have access to the true effects. Therefore, before running the estimation, I write a short function to compute the average treatment effect on the treated (ATET). I will add this information to the estimation results. 
...
We then run the estimation. To do so, I only consider observations within the bandwidth and regress the final test scores on the treatment, the qualification score and their interaction. Note that I include this interaction term to allow more flexibility and to mimic an realistic estimation. Yet, we know that this interaction term does not appear in the DGP. Including it or not do not change the results. Also note that, of course, I do not include the unobserved ability in this model to create an OVB. 
...
We can now run a simulation, combining `generate_data_RDD` and `estimate_RDD`. To do so I create the function `compute_sim_RDD`. This simple function takes as input the various parameters along with a vector of bandwidth sizes, `vect_bw`. If we want to run several simulations with different bandwidths, we can reuse the same data, hence why I allow to passing a vector of bandwidths and not only one bandwidth. The function returns a table with the estimate of the treatment, its p-value and standard error, the true effect and the bandwidth and intensity of the OVB considered ($\gamma$). Note for now, that I do not store the values of the other parameters for simplicity because I consider them fixed over the study.
...
I will run the simulations for different sets of parameters by mapping the `compute_sim_RDD` function on each set of parameters. I thus create a table with all the values of the parameters I want to test `param_rdd`. Note that in this table each set of parameters appears `n_iter` times as we want to run the analysis $n_{iter}$ times for each set of parameters.

```{r set_param_RDD}
fixed_param_RDD <- baseline_param_RDD |> #|> rbind(...)
  mutate(vect_bw = 0.1)
vect_bw <- c(0.067, seq(0.1, 0.9, 0.1))
n_iter <- 2000
...
I then run the simulations by mapping the `compute_sim_RDD` function on `param_rdd`.
...
First, I quickly explore the results. In the following figure, I can see that for small bandwidth estimates are unbiased but imprecise while for large bandwidths estimates are precise but biased.
...
We want to compare $\mathbb{E}\left[ \left| \frac{\widehat{\beta_{RDD}}}{\beta_1}\right|\right]$ and $\mathbb{E}\left[  \left| \frac{\widehat{\beta_{RDD}}}{\beta_1} \right| | \text{signif} \right]$. The first term represents the bias and the second term represents the exaggeration ratio. This terms depend on the true effect size. To enable comparison across simulations and getting terms independent of effect sizes, I also compute the average of the ratios between the estimate and the true effect, conditional on significance. 
...
To analyze the results, we build a unique and simple graph:
...
I can then look in more details to the distribution of the estimates for different bandwidths. Statistically significant estimates, when power is low (*ie* when the bandwidth is small) are located in the tail of the distribution all estimates that is itself centered around the true effect.
...
```{r table_results_RDD, code_folding=TRUE}
summary_sim_rdd |> 
  select(bw, power, type_m) |> 
  kable(
    col.names = c("Bandwidth", "Power", "Exaggeration"),
    digits = 2
  )
```

In the case of the sharp-RD, the confounding-exaggeration trade-off is driven by the bandwidth size and therefore the sample size in this bandwidth. Let's explore the exaggeration of our simulation a sample size consistent with that of the literature. In @jacob_remedial_2004 there are about 5000 observations inside their optimal bandwidth. That represents about 6.7% of their sample. In my simulations, a 0.1 bandwidth has even more observations (6000) and displays particularly large exaggeration: `r round(summary_sim_rdd |> filter(bw == 0.1) |> pull(type_m), 2)`. 

## Further checks

### Representativeness of the estimation

I calibrated my simulations to emulate a typical study from this literature. To further check that the results are realistic, I compare the average Signal-to-Noise Ratio (SNR) of ly regressions to the range of t-stats of an existing study.^[The SNR is the ratio of the true effect over the standard error of the estimator. Yet, since the true effect of the study is not known, I resort to using its point estimate.] The closest study to my simulation is @jacob_remedial_2004. In their main result table, Table 3, the t-stats range from 0.3 to 8. Note that these t-stats may be upper bounds of the actual SNR if there is indeed exaggeration.
...
The precision of my results is therefore in line with precision that can be found in the literature. It is important to acknowledge that my simulation in no means reproduces the study in @jacob_remedial_2004 nor can be used to analyze their results directly. Rather, it combine insights from several studies to mimic a "typical" study from this literature, while remaining conservative with regards to many assumptions, in particular in terms of data generating model (constant and homogenous treatment, ect). Parameters values however are such that they yield a design that can be underpowered, something that can happen in this literature but does not affect every single study.

### Variation in effect size

I then quickly explore whether the same pattern of artificially large significant estimates for small bandwidth sizes emerges on some individual data sets. To do so, I run a few simulations, and plot estimates for each bandwidth size. This pattern indeed appear for some data sets.^[I only display 4 simulation for clarity of exposure but running more simulations confirms that.]

```{r few_sim, fig.asp=1.1, code_folding=TRUE}
...
Interestingly, one can also notice that in some simulations estimates are larger than the true effect and significant for any or most bandwidth sizes. As a consequence, on such data sets, robustness checks may still yield significant and large estimates

```{r robustness, code_folding=TRUE}
ex_data_robust <- baseline_param_RDD |> 
  pmap(generate_data_RDD) |> 
  list_rbind()

ex_data_robust_subset <- ex_data_robust |> 
  slice_sample(n = 10000)

sim_robust <- pmap(list(bw = seq(0.1, 0.9, 0.1)), estimate_RDD, data = ex_data_robust_subset) |> 
  list_rbind() |> 
  mutate(
    signif = ifelse(p_value < 0.05, "Significant", "Non-significant")
  )
...
# ex_data_bw <- ex_data_robust |> 
#   define_bw(0.2) |> 
#   mutate(
#     in_bw = !is.na(treated_bw),
#     id_bw = ifelse(is.na(treated_bw), id, "in bw"),
#     qual_quant = cut_interval(qual, 100),
#     kl = 1
#   ) 
#   
#   
# reg_robust <- feols(data = ex_data_bw,  fml = final ~ treated + qual | id_bw)
# 
# reg_robust |> 
#   summary()
# 
# reg_robust |> ididvar::idid_viz_weights("treated", qual_quant, kl)
```
