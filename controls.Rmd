...
As discussed in the introduction and the math section of the paper, when controlling for confounders may a confounding / exaggeration trade-off is mediated by the proportion of the variation that the control absorbs in the explanatory variable of interest ($x$) as compared to the outcome variable ($y$). Controling for a variable ($w$) is equivalent to partialling out this variable from both $x$ and $y$. Since the variance of the estimator is given by the ratio of the variance of the residuals $\sigma_{y^{\perp x, w}}^2$ over $n$ times the variance off the explanatory variable after partialing out $w$ ($\sigma_{x^{\perp w}}^2$), if controling absorbs more of the variance in $x$ than in $y$, it will increase the variance of the resulting estimator and create exaggeration.
...
ggsave("main_graph_controls_paper.pdf", path = "images", width = 8, height = 4.5)
```
