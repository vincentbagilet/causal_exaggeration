			...
			\subsubsection{Asymptotic distribution}	
				...
				\begin{align*}
            				v_i & =  y_i - \text{x}_i'\bm{\beta}_{\textsc{ovb}} \\
            					...
            					& = \delta w_{i} + u_i - \underbrace{\text{x}_{i}' \mathbb{E}[\text{x}_{i}\text{x}_{i}']^{-1} \mathbb{E}[\text{x}_{i}w_{i}]}_{\text{projection of $w$ on $X$}} \delta\\
            					& =  u_i + \underbrace{\delta w_{i} - \dfrac{\delta\gamma\sigma_{w}^{2}}{\sigma_{x}^{2}} (x_{i} - \mu_{x})}_{\text{Part of $w$ that is orthogonal to $X$}}\\
            				...
            				\Rightarrow \quad \mathbb{E}[v_{i}^{2} | \text{x}_{i}] & = \delta^{2} \mathbb{E}[w_{i}^{2} | \text{x}_{i}] + \underbrace{\mathbb{E}[u_{i}^{2} | \text{x}_{i}]}_{\sigma_u^{2}} + \left( \dfrac{\delta\gamma\sigma_{w}^{2}}{\sigma_{x}^{2}} \right)^{2}  \underbrace{\mathbb{E}[(x_{i} - \mu_{x})^{2} | \text{x}_{i}]}_{(x_{i} - \mu_{x})^{2}}\\
				...
				\textit{~\\Proof to be continued/modified here but I think I should get to $\mathbb{E}[\text{x}_{i}\text{x}_{i}']^{-1} \mathbb{E}[\text{x}_{i}\text{x}_{i}'v_{i}^{2}] \mathbb{E}[\text{x}_{i}\text{x}_{i}']^{-1} = \frac{\delta^{2}\sigma_{w}^{2} + \sigma_{u}^{2}}{\sigma_{x}^{2}}$. This relation seems to hold in simulations.\\}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
		...
		And finally, 
			...
			This variance is close to the one obtained in the case of the naive regression with $\eta_{1}^{2}\sigma_{z}^{2}$ instead of $\sigma_{x}^{2}$ in the denominator. Note that, the following holds:
				...
				As a consequence, $\boxed{\sigma_{\textsc{iv}}^{2} > \sigma_{\textsc{ovb}}^{2}}$. While unbiased, the IV estimator is less precise than the naive estimator. This is a well known and intuitive result that we recovered in a particular setting, tied to the modelling assumptions we made. This result is due to the fact that the IV only uses part of the variation in the endogenous dependent variable, the part that is explained by the instrument. 
				...
				~
				\begin{equation}
					\sigma_{\textsc{iv}}^{2}  = \dfrac{\delta^{2}\sigma_{w}^{2} + \sigma_{u}^{2}}{n \ \sigma_{x}^{2} \rho_{xz}^{2}} \quad \text{and thus} \quad \boxed{\sigma_{\textsc{iv}}^{2}  = \dfrac{\sigma_{\textsc{ovb}}^{2}}{\rho_{xz}^{2}}}
				\end{equation}
