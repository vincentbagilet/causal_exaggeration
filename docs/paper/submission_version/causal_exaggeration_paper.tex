...
\selectlanguage{english}
	...
	\end{center}
		...
		\begin{center}
			\large \textsc{\textbf{Alternative abstract}} \normalsize\\~\\
		\end{center}
...
%%%%%%%%%%%%%%  Exagg ratio  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		\subsection{Properties of the exaggeration ratio}\label{general_exagg}
			...
			Following  \cite{gelman_beyond_2014}, let's define an exaggeration ratio $E$, as the expected value the absolute value of significant estimates over the absolute value of the true effect:
			...
			\cite{lu_note_2019} and \cite{zwet_significance_2021} showed that, for a given size of a test and true effect size, the exaggeration ratio increases with the variance of a unbiased normally distributed estimator. We can easily extend this proof to biased estimators and get that:
		...
		\subsection{Setting and data generating process}\label{maths_dgp}
			...
			Consider a usual linear homoskedastic regression model with an omitted variable. More specifically, for all individual $i \in \{1, ..., n\}$, 
				~
...
%			Note that since $\sigma_{x}^{2}$ and $\sigma_{w}^{2}$ are fixed, reasoning at $b_{\textsc{ovb}} = cst$ is equivalent to considering that $\gamma \delta = \kappa = cst$. Then, noting that $\forall i, u_{i} = y_i - \beta_{0} - \beta_{1}x_{i} - \delta w_{i}$ and computing its variance, we can rewrite the variance of the estimator as a function of fixed variances and one or less varying parameter:
				...
				First, let's study the benchmark against which we are going to compare our causal approaches. Consider the ``naive'' regression of $y$ on $x$ (with $w$ omitted). 
				%Since, $w$ is unobserved, we consider the projection of $y$ on $X$ only, where $X = (\text{x}_{1}', ..., \text{x}_{n}')'$ with $\forall i, \text{x}_{i}' = (1, x_{i})$
%					~
%					\begin{equation}\label{maths_eq_ovb}
%						y = X\bm{\beta}_{\textsc{ovb}} + u_{\textsc{ovb}}
...
%%%%%%%%%%%%%%  CTRL  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			...
			\subsubsection{Controlled regression}
				...
				Next, let's turn to the ``ideal'' case in which no variable is omitted, \textit{i.e.} we control for the omitted variable $w$ and thus partial out confounders. The model considered accurately represents the DGP.
%				~
%				\begin{equation}\label{maths_eq_ovb}
...
%%%%%%%%%%      SIM    Controls        %%%%%%%%%%%%%%%
		 ...
		 \subsection{Controlling for confounders}
     			...
     			\paragraph{Intuition.} To identify a causal effect and avoid the risk of confounders, an ``ideal'' approach would be to control for them. % by including additional variables in the regression model to eliminate endogenous variation. In the ideal case, we would control for the omitted variable and for this only. 
...
%%%%%%%%%%      SIM    Matching        %%%%%%%%%%%%%%%		
 ...
 \subsection{Matching}
			...
			\paragraph{Intuition.} Another approach to retrieving a causal effect in a situation of selection on observables is to use matching. This method defines ``counterfactuals'' for treated units by picking comparable units in the untreated pool. 
...
\section{Next steps} \label{next_steps}
	...
	\textit{For each task, I write down what section of the paper it pertains to (S = Simulations, M = Maths, R = Recommendations, W = Writing). Question marks mean that I ma not certain that this task is necessary.}\\
	...
	\begin{enumerate}
		...
		\item (S) Verify the variances of the residuals in my simulations: are they similar to those in the published papers?
		\item (S) Calibrate simulations for FEs/controls
		\item (W) Using \cite{brodeur_methods_2020}, try to see if I can compare exaggeration across strategies (by comparing how the two ingredients in the recipe for exaggeration vary across strategy: publication bias and statistical power)
		...
		\item (R) Add recommendation to compute a MDE (?)
		\item (W) Add the simulation codes (my RMarkdown files) to the appendix
