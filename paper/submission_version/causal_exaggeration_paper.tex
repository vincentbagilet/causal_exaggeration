...
%-----------------------------------------------------------------------------
	...
	\section{Mathematical derivation} \label{maths}
		...
		In this section, I formally prove the existence of the confounding / exaggeration trade-off  and describe its drivers. To do so, I first recall a general definition of the exaggeration ratio and discuss its evolution with parameters values. Then, I derive the exaggeration ratio for a series of estimators under a set of simplifying assumptions: in the presence of an omitted variable bias, when controlling for the omitted variable, when instrumenting it or running a reduced form analysis. Finally, I show that, for any magnitude of OVB, the overall bias, combining exaggeration and OVB, can be greater when controlling for confounders using a causal inference methods than not. A more general and detailed proof, relaxing some assumptions such as normality, is available on the \href{https://vincentbagilet.github.io/causal_exaggeration/Maths/math_causal_exaggeration.pdf}{companion website}.
		
		\subsection{Setting and data generating process}
		
			Consider an usual normal linear homoskedastic regression model with an omitted variable. For all individual $i \in \{1, ..., n\}$, assume that: 
				~
