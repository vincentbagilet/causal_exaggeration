...
*In this paper, we show that, combined with current academic publication practices, front line empirical methods, while efficient to measure how a factor causes another, might in some cases produce effects larger than what they should be.*
...
The set of methods mentioned above enables to convincingly measure how a factor causes another. However, these methods are also more subject to the publication issue described above. In some cases, they may be more likely than conventional methods to produce effect sizes that are too large. In this paper, we show that there is a trade-off between measuring an actual causal impact and exaggerating effect sizes due to the publication problem discussed above.

To do so we first build simulations, generating fake data representative of real life situations. We have to rely on simulations because to evaluate how far the result of an analysis is from the true effect, one needs to know this true effect. In real life cases, the true effect is never known (otherwise one would not build a study to estimate it). Simulations enables us to define the "true effect" ourselves. We then derive a mathematical proof of the existence trade-off described above.

To avoid published effects to exaggerate true effect sizes, we also advocate computing and reporting a very simple calculation that may help evaluate the risk of exaggeration. We develop a tool to explicitly visualize the factor that drives the trade-off. This tool enables practitioners to gauge where their study lies with respect to this trade-off.
